...
# Global Stop Event added for proper scoping
...
def _signal_handler(signame):
    ...
    """
    logger.info(f"Received signal {signame}. Attempting shutdown...")
    stop_event.set()  # Mark stop event
    # IMPORTANT:
    # Do NOT call loop.stop() when running under asyncio.run(...).
    # asyncio.run expects the loop to keep running long enough to cancel/await tasks.
    logger.info("Signal handled; waiting for tasks to exit cleanly.")

async def main():
    logger.info("Starting bot...")

    # Get the running event loop (safe inside asyncio.run)
    ...
    for signame in ('SIGINT', 'SIGTERM'):
        try:
            loop.add_signal_handler(
                ...
                lambda signame=signame: _signal_handler(signame)
            )
            logger.info(f"Signal handler registered for {signame}")
        except NotImplementedError:
            # Windows/Proactor event loop often does not support add_signal_handler.
            # Fall back to signal.signal and wake the asyncio loop thread-safely.
            logger.warning(f"loop.add_signal_handler not supported for {signame}; using signal.signal fallback.")
            sig = getattr(signal, signame, None)
            if sig is not None:
                def _win_handler(_signum, _frame, _sn=signame):
                    try:
                        loop.call_soon_threadsafe(_signal_handler, _sn)
                    except Exception:
                        # As a last resort, just set the flag
                        stop_event.set()
                try:
    ...
    try:
        ...
        tasks = [
            asyncio.create_task(heartbeat_task(stop_event, interval=5), name="heartbeat"),
            # Add other long-running tasks here as create_task(..., name="...")
        ]

        # Wait until a shutdown signal sets the event
        await stop_event.wait()
    except Exception as e:
        logger.error(f"Error in main tasks: {e}")
    finally:
        logger.info("Stopping bot...")
        stop_event.set()  # Ensure all tasks observe this
        # Cancel outstanding tasks (clean exit)
        try:
            for t in asyncio.all_tasks(loop):
                if t is asyncio.current_task(loop=loop):
                    continue
                t.cancel()
            await asyncio.gather(*[t for t in asyncio.all_tasks(loop) if t is not asyncio.current_task(loop=loop)],
                                 return_exceptions=True)
        except Exception:
            logger.debug("Task cancellation during shutdown raised", exc_info=True)        
...
async def _build_live_shortlist(
    *,
    ...
    # IMPORTANT: do NOT destroy the raw discovery set used by the GUI.
    # We keep a raw copy for persistence, and apply the filter only to the pipeline list.
    merged_raw = list(merged)  # <- raw snapshot for UI persistence
    ...
    try:
        if enforce_scoring_hard_floor:
            ...
            merged = kept  # <- filtered pipeline list ONLY
            after_n = len(merged)
    ...
    try:
        ...
        # Persist the RAW discovered set (pre hard_floor / pre-cut) so the GUI can show
        # large caps and also show what got rejected later.
        for _t in merged_raw:
...
async def heartbeat_task(stop_event: asyncio.Event, interval: int = 5) -> None:
    ...
    try:
        ...
        raise
    except Exception as e:  # Use `as e` to capture the exception object
...
async def main() -> None:
    ...
    try:  # ==== OUTER TRY ====
        ...
        def _signal_handler(signame):
            logger.info(f"Received signal {signame}. Attempting shutdown...")
            stop_event.set()  # For asyncio tasks using stop_event
            try:
                asyncio.get_running_loop().stop()  # Stop the active event loop
            except Exception:
                pass
            sys.exit(0)  # Force process exit if event loop stop doesn't suffice
