...
Updated as of December 20, 2025: Jupiter API endpoints updated to V6; Dexscreener API updated to latest endpoint.
...
async def get_token_market_cap(
    ...
    try:
        ...
        if status == 200:
            pairs = json.loads(text)  # Now a list of pairs
...
async def get_token_price_in_sol(
    ...
    try:
        ...
        if status == 200:
            pairs = json.loads(text)  # list of pairs
            chosen = None
            for p in pairs:
                try:
...
async def get_jupiter_quote(
    ...
    *,
    ...
    """
    Jupiter quote: GET /swap/v1/quote with dual-stack → IPv4 fallback.
    ...
    """
    try:
        # Enforce liquidity pre-check if token_data provided
        ...
        # Updated Jupiter quote endpoint
        ...
        # Include x-api-key if provided
        jup_key = (os.getenv("JUP_API_KEY") or "").strip()
        ...
        if status != 200:
            logger.error("Jupiter quote failed: HTTP %s, %s", status, (text or "")[:300])
            return None, text
        ...
        log_error_with_stacktrace(f"Error fetching Jupiter quote for {input_mint} -> {output_mint}", e)
        return None, str(e)
...
async def build_jupiter_swap(
    ...
    """
    Jupiter swap: POST /swap/v1/swap with dual-stack → IPv4 fallback.

    Returns:
      - JSON object with 'swapTransaction' (base64) and metadata.
    """
    try:
        # Updated Jupiter swap endpoint
        ...
        status, text = await _fetch_text_with_ipv4_fallback(
            "POST", url, data=json.dumps(body), headers=headers, timeout=SHORT_TIMEOUT, session=session
        )

        if status != 200:
            logger.error("Jupiter swap build failed: HTTP %s, %s", status, (text or "")[:300])
            return None, text

        data = json.loads(text)
        if "swapTransaction" not in data:
            logger.error("Jupiter swap build lacks swapTransaction: %s", (text or "")[:300])
            return None, "missing swapTransaction"

        logger.debug("Jupiter swap built OK: lastValidBlockHeight = %s", data.get("lastValidBlockHeight"))
        return data, None

    except Exception as e:
        log_error_with_stacktrace("Error building Jupiter swap", e)
        return None, str(e)
