...
# ---- Add defensive imports for preflight + ATA/token-program helpers ----
try:
    # preflight helper added in trading_preflight.py
    from .trading_preflight import _preflight_buy_checks
except Exception:
    _preflight_buy_checks = None

try:
    # helpers that may live in market_data or another module in the repo
    from .market_data import get_token_program_id, get_associated_token_address  # type: ignore
except Exception:
    get_token_program_id = None
    get_associated_token_address = None

# spl-token program constant fallback (only used for informative logging)
try:
    from spl.token.constants import TOKEN_PROGRAM_ID as SPL_TOKEN_PROGRAM_ID  # type: ignore
except Exception:
    SPL_TOKEN_PROGRAM_ID = None
...
# Dynamic ATA helper for Token-2022 support
try:
    from .market_data import get_associated_token_address
except Exception:
    try:
        from solana_trading_bot_bundle.trading_bot.market_data import get_associated_token_address
    except Exception:
        async def get_associated_token_address(*args, **kwargs):
            raise RuntimeError("get_associated_token_address not available — check market_data.py")
...
# ----- DB helper binding + fallback + dry-run persistence helpers -----
...
if _db is None:
    ...
    # Eagerly initialize shared DB connection in background (best-effort).
    try:
        if _db is not None:
            # prefer an explicit initializer if present; fall back to _ensure_shared_conn
            init_fn = getattr(_db, "init_shared_db", None) or getattr(_db, "_ensure_shared_conn", None)
            if callable(init_fn):
                try:
                    # Schedule init when an event loop is available/running (non-blocking).
                    loop = None
                    try:
                    ...
                    if loop and loop.is_running():
                        # schedule as background task so import doesn't block
                        loop.create_task(init_fn())
                    else:
                        # no running loop at import time; skip eager init (first DB user will initialize)
                        pass
                except Exception:
                    logger.debug("Eager DB init scheduling failed (continuing): %s", traceback.format_exc())
    except Exception:
        # best-effort only; don't fail imports
        logger.debug("Eager DB init scheduling top-level failure (ignored): %s", traceback.format_exc())
...
_TOKEN_PROGRAM_LEGACY = "TokenkegQfeZyiNwAJbNbGKPFXCWuBvf9Ss623VQ5DA"
_TOKEN_PROGRAM_2022 = "TokenzQdBNbLqP5VEhdkAS6EPFLC1PHnBqCXEpPxuEb"

async def _detect_token_program_id(mint_addr: str, solana_client: Any, *, logger: Optional[logging.Logger] = None, timeout_s: float = 3.0) -> str:
    """
    Best-effort: ask RPC for the mint account info and return the account owner (token program id).
    - Returns a string pubkey of the program owner, or the legacy token program id as a safe default.
    - Non-blocking and tolerant: never raises (returns fallback on error).
    """
    log = logger or globals().get("logger")
    try:
        if not mint_addr:
            return _TOKEN_PROGRAM_LEGACY
        # Some callers might pass Pubkey already
        try:
            mint_pub = _Pubkey(mint_addr)
        except Exception:
            try:
                mint_pub = _Pubkey(str(mint_addr))
            except Exception:
                return _TOKEN_PROGRAM_LEGACY

        # If solana_client has get_account_info async method, call with timeout
        if solana_client is None:
            return _TOKEN_PROGRAM_LEGACY

        try:
            coro = solana_client.get_account_info(mint_pub)
            if asyncio.iscoroutine(coro):
                try:
                    # bounded wait so we don't stall buy loop
                    info_res = await asyncio.wait_for(coro, timeout=timeout_s)
                except asyncio.TimeoutError:
                    try:
                        if log:
                            log.debug("detect_token_program_id: RPC timeout for mint %s", mint_addr)
                    except Exception:
                        pass
                    return _TOKEN_PROGRAM_LEGACY
            else:
            ...
            try:
                if log:
                    log.debug("detect_token_program_id: get_account_info failed for %s: %s", mint_addr, e)
            except Exception:
                pass
            return _TOKEN_PROGRAM_LEGACY

        # Try to extract owner (varies by client wrapper; handle common shapes)
        try:
            # solana-py AsyncClient returns a RPCResponse-like with .value.owner or dict
            val = getattr(info_res, "value", info_res)
            if val:
                # may be a dict with 'owner' key
                if isinstance(val, dict):
                    owner = val.get("owner") or val.get("ownerAddress") or val.get("owner_pubkey")
                else:
                    # object with attribute owner or owner_pubkey
                    owner = getattr(val, "owner", None) or getattr(val, "owner_pubkey", None)
                if owner:
                    return str(owner)
        except Exception:
            pass

        # Fallback: if the account info contains 'data' we cannot infer owner reliably here
        return _TOKEN_PROGRAM_LEGACY
    except Exception:
        try:
            if logger:
                logger.debug("_detect_token_program_id unexpected error for %s", mint_addr, exc_info=True)
        except Exception:
            pass
        return _TOKEN_PROGRAM_LEGACY
...
async def _resolve_buy_amount_sol(
    *,
    ...
    def _normalize_return(v: Any) -> float:
        # Normalize different return shapes to a single SOL amount (float)
        try:
    ...
    # Build kwargs matching target fn parameter names (best-effort)
    def _build_kwargs(fn):
        params = set()
        try:
            params = set(signature(fn).parameters.keys())
        except Exception:
            # If we can't introspect, fallback to empty set and use default modern kwargs later
            params = set()

        kw: dict[str, Any] = {}

        # token param name can be "token" or "token_data"
        if "token" in params:
            kw["token"] = token
        elif "token_data" in params:
            kw["token_data"] = token
        # also tolerate single-name "t" or "token_info" if needed in future (not required now)

        if "wallet_balance" in params:
            kw["wallet_balance"] = wallet_balance
        if "sol_price" in params:
            kw["sol_price"] = sol_price
        if "config" in params:
            kw["config"] = config

        # If introspection failed (params empty), prefer the modern/common kw form
        if not params:
            kw = {
                "token": token,
    ...
    # Async call wrapper with fallbacks
    async def _call_async(fn):
        last_exc = None
        # 1) Best-effort keyword call (autodetect param names)
        try:
            return await fn(**_build_kwargs(fn))
        except TypeError as e:
            last_exc = e
        except Exception:
            # non-TypeError exceptions should surface (they're not signature mismatches)
            raise

        # 2) Positional fallbacks (common historical signatures)
        positional_candidates = (
            (token, wallet_balance, sol_price, config),  # common modern positional
            (wallet_balance, sol_price, config),         # some variants without token
            (config, sol_price),                         # very old legacy
        )
        ...
        # Nothing worked: re-raise last TypeError for visibility
        if last_exc:
            raise last_exc
        # defensive final attempt
        return await fn(**_build_kwargs(fn))

    # Sync call wrapper with the same fallbacks
    def _call_sync(fn):
        last_exc = None
        try:
            return fn(**_build_kwargs(fn))
        except TypeError as e:
            last_exc = e
        except Exception:
            raise
        ...
        for args in (
            (token, wallet_balance, sol_price, config),
            (wallet_balance, sol_price, config),
            (config, sol_price),
        ):
            try:
                return fn(*args)
    ...
    # Execute and normalize return into a SOL float
    try:
        if iscoroutinefunction(get_buy_amount_fn):
            raw = await _call_async(get_buy_amount_fn)
        else:
            raw = _call_sync(get_buy_amount_fn)
    except Exception:
        # Bubble unexpected exceptions so operator can see misbehaving sizing functions
        raise

    return _normalize_return(raw)
...
async def _load_persisted_shortlist_from_db(
    ...
    - This runs the existing (blocking) sqlite3 work in a thread via asyncio.to_thread
      so it does not block the event loop.
    - Returns [] if the DB is missing, empty, too old, or too small.
    - Callers must await this function (e.g. `eligible_tokens_from_db = await _load_persisted_shortlist_from_db(...)`).
    ...
    def _blocking_load(config_inner, max_age_inner, min_count_inner):
        """Blocking implementation that runs inside a thread."""
        try:
            db_path = _db_path_from_config(config_inner)
        except Exception:
            # If path resolution fails, behave as "no DB"
        ...
        try:
            ...
            # If .exists() fails for some reason, treat as missing DB
        ...
        try:
            ...
            def _get_ts(row: sqlite3.Row) -> float | None:
                # Try common timestamp fields tolerant to string/int/float
                try:
            ...
            for r in rows:
                try:
                    ...
                    # Copy over common numeric fields when present (tolerant: avoid KeyError)
                    for k in (
...
async def _build_live_shortlist(
    *,
    ...
    This version adds configurable discovery-level liquidity/volume gating so
    extremely low-liquidity tokens are filtered out before scoring/shortlist.
    """
    ...
    # Discovery-level hard filters (configurable)
    ...
    # Config keys (discovery.*) with env fallbacks
    min_liquidity_usd = float((disc.get("min_liquidity_usd") or _env_float("MIN_LIQUIDITY_USD", 200.0)))
    min_volume_24h_usd = float((disc.get("min_volume_24h_usd") or _env_float("MIN_VOLUME_24H_USD", 50.0)))
    reject_low_liq = bool(disc.get("reject_low_liq", True))
    # Optionally require at least N buyers / makers if provider exposes it (not always present)
    min_buyers = int((disc.get("min_buyers", os.getenv("MIN_BUYERS") or 0) or 0))

    # Small helper to coerce numeric-ish values defensively
    def _to_float_loose(x: Any, default: float = 0.0) -> float:
        try:
            if x is None:
                return float(default)
            if isinstance(x, (int, float)):
    ...
    # ---------- NEW: discovery-level liquidity/volume pre-filter ----------
    # This rejects tokens with tiny pools or near-zero trading activity BEFORE scoring.
    try:
        if reject_low_liq:
            before_n = len(merged)
            ...
            for t in merged:
                try:
                    ...
                    liq = _to_float_loose(t.get("liquidity") or t.get("dex_liquidity") or t.get("ray_liquidity_usd") or 0.0, 0.0)
                    vol = _to_float_loose(
                        t.get("v24hUSD") or t.get("volume_24h") or t.get("volume24h") or t.get("dex_volume_24h") or 0.0,
                        0.0,
                    )
                    # optional buyers field used by some providers (Dexscreener shows "buyers"/"sellers")
                    buyers = int(t.get("buyers") or t.get("num_buyers") or 0) if t.get("buyers") is not None else 0

                    # quick rejects: zero/None price, extremely small liquidity, or tiny 24h volume
                    price = _to_float_loose(t.get("price") or t.get("lastPrice") or 0.0, 0.0)
                    if not price or price <= 0:
                        t["_reject_reason"] = "no_price"
                        rejected.append(t)
                        continue

                    if liq < float(min_liquidity_usd) or vol < float(min_volume_24h_usd):
                        # if buy-count is present, require a minimum if configured
                        if min_buyers and buyers and buyers >= min_buyers:
                            kept.append(t)  # allow if buyers indicate real activity
                        else:
                            t["_reject_reason"] = "low_liquidity"
                            t["_liquidity_usd"] = liq
                            t["_v24h_usd"] = vol
                            t["_buyers"] = buyers
                            rejected.append(t)
                        continue
                    ...
                    # on extraction error, keep conservative (include token) but log debug
                    try:
                        logger.debug("Liquidity filter extraction failed for token %s", t.get("address"), exc_info=True)
                    except Exception:
                        pass
                    kept.append(t)
            ...
            merged = kept
            after_n = len(merged)
            # Log diagnostics: counts and top rejected sample
            try:
                logger.info("Discovery prefilter: kept=%d rejected=%d (min_liq_usd=%.2f min_vol24h_usd=%.2f min_buyers=%d)",
                            after_n, (before_n - after_n), float(min_liquidity_usd), float(min_volume_24h_usd), int(min_buyers))
                if rejected:
                    sample = rejected[:5]
                    samp_str = ", ".join(f"{(s.get('symbol') or s.get('address') or 'UNK')}({s.get('_liquidity_usd'):.0f}/${s.get('_v24h_usd'):.0f})" if s.get('_liquidity_usd') is not None else str(s.get('address')) for s in sample)
                    logger.debug("Discovery prefilter sample rejected: %s", samp_str)
            except Exception:
                pass
    except Exception:
        try:
            logger.debug("Discovery prefilter failed (continuing):", exc_info=True)
        except Exception:
            pass
    # --------------------------------------------------------------------
    ...
    try:
        ...
        logger.debug("Persist discovered tokens normalization failed (continuing)", exc_info=True)
    ...
    # --- HARD-FLOOR PRUNING ---
    try:
        hf_cfg = (config or {}).get("scoring", {}).get("hard_floor", {}) or {}
        if isinstance(hf_cfg, dict) and bool(hf_cfg.get("enable", False)):
            min_liq = float(hf_cfg.get("min_liquidity_usd", 0) or 0)
            min_vol = float(hf_cfg.get("min_volume_24h_usd", 0) or 0)
            min_mc = float(hf_cfg.get("min_market_cap_usd", 0) or 0)
            max_mc = float(hf_cfg.get("max_market_cap_usd", 0) or 0)
            allow_whitelist = bool(hf_cfg.get("allow_whitelist", True))

            kept = []
            rejected = []
            for t in (tokens_for_selection or []):
                try:
                    # Whitelist bypass
                    addr = (t.get("address") or "").strip()
                    sym = (t.get("symbol") or "").strip()
                    if allow_whitelist and (addr in WHITELISTED_TOKENS or sym in set(WHITELISTED_TOKENS.values())):
                        kept.append(t)
                        continue

                    # Defensive numeric coercion (reuse local helper if present)
                    def _to_float_loose_local(x, d=0.0):
                        try:
                            if x is None:
                                return float(d)
                            if isinstance(x, (int, float)):
                    ...
                    liq = _to_float_loose_local(t.get("liquidity") or t.get("dex_liquidity") or t.get("ray_liquidity_usd") or 0.0)
                    vol = _to_float_loose_local(t.get("v24hUSD") or t.get("volume_24h") or t.get("volume24h") or t.get("dex_volume_24h") or 0.0)
                    mc_val = _to_float_loose_local(t.get("market_cap") or t.get("mc") or t.get("fdv") or 0.0)
                    ...
                    if min_vol and vol < min_vol:
                        fail = True
                    if min_mc and mc_val < min_mc:
                        fail = True
                    if max_mc and max_mc > 0 and mc_val > max_mc:
                        fail = True

                    if fail:
                        t["_reject_reason"] = "hard_floor"
                        t["_liquidity_usd"] = liq
                        t["_v24h_usd"] = vol
                        t["_market_cap_usd"] = mc_val
                        rejected.append(t)
                    else:
                        kept.append(t)
                except Exception:
                    # On error, be conservative and keep the token (don't accidentally drop candidates)
                    kept.append(t)

            tokens_for_selection = kept

            try:
                logger.info(
                    "Hard-floor pruning: kept=%d rejected=%d (min_liq=%.2f min_vol=%.2f min_mc=%.2f max_mc=%.2f allow_whitelist=%s)",
                    len(kept), len(rejected), float(min_liq), float(min_vol), float(min_mc), float(max_mc), allow_whitelist
                )
                if rejected:
                    # sample for debug
                    samp = rejected[:5]
                    samp_str = ", ".join(
                        f"{(s.get('symbol') or s.get('address') or 'UNK')} liq={s.get('_liquidity_usd', 0):.0f} vol={s.get('_v24h_usd',0):.0f} mc={s.get('_market_cap_usd',0):.0f}"
                        for s in samp
                    )
                    logger.debug("Hard-floor sample rejected: %s", samp_str)
            except Exception:
                pass
    except Exception:
        try:
            logger.debug("Hard-floor pruning failed (continuing):", exc_info=True)
        except Exception:
            pass
    # --- end hard-floor pruning ---
...
async def _preflight_buy_checks(
    token: dict,
    *,
    config: dict | None = None,
    session: "aiohttp.ClientSession" | None = None,
    ...
    jup_probe_lamports: int | None = None,
    blacklist_on_fail: bool = True,
) -> tuple[bool, str | None]:
    """
    Best-effort preflight checks before attempting a buy.

    Returns (ok: bool, reason: Optional[str]).
    - ok == True -> it's OK to proceed to get an actual quote / execute swap.
    - ok == False -> skip buy; reason explains why.

    Resolution order for min_liquidity_usd / min_volume_24h_usd:
      1) explicit args passed to this function
      2) config["discovery"][...]
      3) env JUP_MIN_LIQUSD / JUP_MIN_VOL24H
      4) defaults: min_liquidity_usd=1000.0, min_volume_24h_usd=100.0

    jup_probe_lamports defaults to:
      - env JUP_PROBE_LAMPORTS (if set)
      - else 100_000 lamports (0.0001 SOL)
    """
    try:
        # Resolve config/env defaults
        cfg = config or {}
        disc = (cfg.get("discovery") or {}) if isinstance(cfg, dict) else {}

        try:
            env_min_liq = float(os.getenv("JUP_MIN_LIQUSD")) if os.getenv("JUP_MIN_LIQUSD") else None
        except Exception:
            env_min_liq = None
        try:
            env_min_vol = float(os.getenv("JUP_MIN_VOL24H")) if os.getenv("JUP_MIN_VOL24H") else None
        except Exception:
            env_min_vol = None

        min_liq = float(min_liquidity_usd) if (min_liquidity_usd is not None) else float(
            disc.get("min_liquidity_usd") or env_min_liq or 1000.0
        )
        min_vol = float(min_volume_24h_usd) if (min_volume_24h_usd is not None) else float(
            disc.get("min_volume_24h_usd") or env_min_vol or 100.0
        )

        # Resolve probe lamports
        try:
            env_probe = int(os.getenv("JUP_PROBE_LAMPORTS")) if os.getenv("JUP_PROBE_LAMPORTS") else None
        except Exception:
            env_probe = None
        probe_lamports = int(jup_probe_lamports or env_probe or 100_000)

        # Safely extract numeric liquidity & volume candidates from token
        try:
            liquidity = float(token.get("liquidity") or token.get("dex_liquidity") or token.get("liq") or token.get("liquidity_usd") or 0.0)
        except Exception:
            liquidity = 0.0
        try:
            volume24 = float(token.get("volume_24h") or token.get("v24hUSD") or token.get("dex_volume_24h") or token.get("volume24h") or 0.0)
        except Exception:
            volume24 = 0.0

        addr = token.get("address") or token.get("token_address") or token.get("mint") or "<unknown>"

        # 0) If Jupiter flagged offline this cycle, skip further probes
        try:
            if _JUP_OFFLINE:
                reason = "jupiter_offline_cycle"
                logger.info("Preflight skip %s: %s", addr, reason)
                return False, reason
        except Exception:
            pass

        # 1) Liquidity/volume gating
        if liquidity < min_liq or volume24 < min_vol:
            reason = f"insufficient_liquidity_or_volume (liq={liquidity:.2f} < {min_liq:.2f} or vol24h={volume24:.2f} < {min_vol:.2f})"
            try:
                logger.info("Preflight reject %s: %s", addr, reason)
            except Exception:
                pass
            if blacklist_on_fail:
                try:
                    await db_add_to_blacklist(addr, reason=reason)
                except Exception:
                    pass
            return False, reason

        # 2) Jupiter tiny-quote probe: prefer WSOL -> token (WSOL mint canonical)
        try:
            input_mint = (cfg.get("jupiter") or {}).get("input_mint") or os.getenv("JUP_INPUT_MINT") or "So11111111111111111111111111111111111111112"
            output_mint = token.get("address") or token.get("token_address") or token.get("mint")
            if not output_mint:
            ...
            # Call the existing small-probe helper
            try:
                ok, err = await _is_jupiter_tradable(
                    input_mint=input_mint,
                    output_mint=output_mint,
                    lamports=probe_lamports,
                    user_pubkey=(user_pubkey or (getattr(token_balance_cache, "user_pubkey", None) if "token_balance_cache" in globals() else None)),
                    session=session,
                    slippage_bps=int(slippage_bps or 50),
                )
            except Exception as e:
                ok, err = False, f"probe_exception:{str(e)}"

            if not ok:
                reason = f"jupiter_probe_failed:{err or 'no_quote'}"
                try:
                    logger.info("Preflight reject %s: %s (probe_lamports=%d)", addr, reason, probe_lamports)
                except Exception:
                    pass
                if blacklist_on_fail:
                    try:
            ...
            # If anything unexpected happens in the probe, be conservative and skip the buy
            reason = f"probe_unexpected:{str(e)}"
            try:
                logger.info("Preflight unexpected failure for %s: %s", addr, reason)
            except Exception:
                pass
            if blacklist_on_fail:
                try:
        ...
        # Defensive fallback: don't block trading on helper failure, but signal a conservative skip
        try:
            logger.debug("Preflight helper crashed for %s", token.get("address"), exc_info=True)
        except Exception:
            pass
        return False, "preflight_internal_error"
                ...
                try:
                    ...
                    try:
                        if not candidates:
                            ...
                            # Local helpers to read config/env with fallbacks (unchanged semantics)
                            def _cfg_get(cfg: dict | None, key: str, env: str | None = None, default: Any = None):
                                """
                                Resolve a value from:
                                1) config dict (supports dot-separated nested keys)
                                2) environment variable (if env provided)
                                3) default
                                Returns the raw value (not cast).
                                """
                                # 1) try nested config lookup (dot-separated)
                                try:
                                    if cfg and key:
                                        cur = cfg
                            ...
                            def _cfg_int(cfg: dict | None, key: str, env: str | None, default: int) -> int:
                                try:
                                    v = _cfg_get(cfg, key, env, default)
                                    if v is None:
                                        return int(default)
                                    if isinstance(v, int):
                                        return v
                                    ...
                                    try:
                                        return int(os.getenv(env or "", str(default)))
                                    except Exception:
                                        return int(default)


                            def _cfg_float(cfg: dict | None, key: str, env: str | None, default: float) -> float:
                                try:
                                    v = _cfg_get(cfg, key, env, default)
                                    if v is None:
                                        return float(default)
                                    ...
                                    try:
                                        return float(os.getenv(env or "", str(default)))
                                    except Exception:
                                        return float(default)


                            def _cfg_bool(cfg: dict | None, key: str, env: str | None, default: bool = False) -> bool:
                                """
                                Interpret booleans from config or env. Accepts real booleans in config,
                                and string env values like "1", "true", "yes", "on".
                                """
                                try:
                                    v = _cfg_get(cfg, key, env, None)
                                    if v is None:
                                        # fallback to env again just to be safe (redundant but explicit)
                                        ev = os.getenv(env or "")
                                        if ev is not None and ev != "":
                                    ...
                                    if s in ("0", "false", "no", "off", "n"):
                                        return False
                                    # Fallback: truthiness of the string / numeric
                                    try:
                                        # numeric strings -> non-zero -> True
                                        f = float(s.replace(",", ""))
                                        return f != 0.0
                                    except Exception:
                                        return bool(s)
                            ...
                            if not sol_price:
                                ...
                                if not buy_amount_sol or float(wallet_balance or 0.0) <= 0.0:
                                    ...
                                    # preflight probe options: allow config to turn on on-chain reserve checks
                                    preflight_onchain_probe = _cfg_bool(tsec, "preflight_onchain_reserve_probe", "PRE_FLIGHT_ONCHAIN_PROBE", False)
                                    # multiplier: required pool liquidity (USD) relative to our buy
                                    preflight_pool_liq_multiplier = float(tsec.get("preflight_pool_liq_multiplier", os.getenv("PRE_FLIGHT_POOL_LIQ_MULTIPLIER", 5.0)))
                                    ...
                                    for tok in candidates:
                                        ...
                                        try:
                                            token_addr = tok.get("address") or tok.get("token_address") or tok.get("mint")
                                            ...
                                            # 1) Use existing _preflight_buy_checks helper if present (preferred)
                                            try:
                                                helper = globals().get("_preflight_buy_checks")
                                                if callable(helper):
                                                    pre_ok, pre_reason = await helper(
                                                        tok,
                                                        config=config,
                                                        session=session,
                                                        user_pubkey=user_pubkey,
                                                    ...
                                                    if not pre_ok:
                                                        _log_skip(symbol, token_addr, f"preflight:{pre_reason}")
                                                        continue
                                            except Exception:
                                                # If the packaged helper raised or is incompatible for this run, fall back to local lightweight probe below.
                                                logger.debug("packaged _preflight_buy_checks failed or not compatible for %s; will run inline probes", token_addr, exc_info=True)
                                                pre_ok = True
                                                pre_reason = None

                                            # 2) If no packaged helper or it returned True but config wants extra on-chain probe, run a conservative on-chain reserve probe
                                            try:
                                                if preflight_onchain_probe:
                                                    # Use fetch_raydium_pool_for_mint if available (best-effort)
                                                    fr = globals().get("fetch_raydium_pool_for_mint")
                                                    if callable(fr):
                                                        try:
                                                            pool_info = await fr(token_addr, session=session)
                                                        except TypeError:
                                                            # Some implementations may accept only (mint,)
                                                            pool_info = await fr(token_addr)
                                                        if pool_info:
                                                            # pool_info may contain reserves or liquidity; attempt safe reads
                                                            pool_liq_usd = None
                                                            try:
                                                                pool_liq_usd = float(pool_info.get("liquidityUsd") or pool_info.get("liquidity") or 0.0)
                                                            except Exception:
                                                                pool_liq_usd = None
                                                            # estimate our buy in USD
                                                            buy_usd = float(per_token_buy_amount_sol or 0.0) * float(sol_price_usd or sol_price or 0.0)
                                                            # require pool liquidity to be meaningfully larger than our buy
                                                            if pool_liq_usd is not None and pool_liq_usd > 0:
                                                                if buy_usd * preflight_pool_liq_multiplier > pool_liq_usd:
                                                                    _log_skip(symbol, token_addr, f"onchain_pool_liquidity_too_small: pool_usd={pool_liq_usd:.2f} buy_usd={buy_usd:.2f}")
                                                                    continue
                                                            else:
                                                                # if pool data lacks liquidity, do not fail the probe — rely on Jupiter quote probe instead
                                                                logger.debug("fetch_raydium_pool_for_mint returned pool_info but no liquidity field for %s", token_addr)
                                                        else:
                                                            logger.debug("fetch_raydium_pool_for_mint returned no pool for %s — skipping onchain_probe fallback", token_addr)
                                                    else:
                                                        logger.debug("No fetch_raydium_pool_for_mint available; skipping onchain probe for %s", token_addr)
                                            except Exception:
                                                logger.debug("On-chain reserve probe failed (non-fatal) for %s", token_addr, exc_info=True)

                                            # 3) As a last check (or if helper missing), try a tiny Jupiter quote probe inline to ensure a route exists.
                                            #    This catches "COULD_NOT_FIND_ANY_ROUTE" quickly and avoids executing a swap that won't route.
                                            try:
                                                # If packaged probe already ran above we would have continued on fail; doing an inline probe only if we don't
                                                # already have a packaged preflight or if packaged returned ambiguous results.
                                                need_inline_probe = True
                                                # If the packed helper set pre_reason/ pre_ok, and pre_ok True, we can still optionally probe if explicit flag
                                                if helper is not None and callable(helper):
                                                    # if the helper returned True/False already, skip inline unless config explicitly asks
                                                    need_inline_probe = bool(tsec.get("always_jupiter_quote_probe", False))
                                                if need_inline_probe:
                                                    # Build tiny quote for WSOL -> token with buy amount in lamports
                                                    probe_amount = int(max(1, int(per_token_buy_amount_sol * LAMPORTS_PER_SOL_LOCAL)))
                                                    try:
                                                        probe_quote, probe_err = await _safe_get_jupiter_quote(
                                                            input_mint=WSOL,
                                                        ...
                                                        if probe_err or not probe_quote:
                                                            _log_skip(symbol, token_addr, f"preflight_jupiter_no_route:{probe_err}")
                                                            continue
                                                        # Optional: examine probe_quote.priceImpactPct or outAmount to detect extreme slippage
                                                        try:
                                                            pi = float(probe_quote.get("priceImpactPct") or 0.0)
                                                            max_price_impact = float((tsec.get("preflight_max_price_impact_pct") or 50.0))
                                                            if abs(pi) > max_price_impact:
                                                                _log_skip(symbol, token_addr, f"preflight_jupiter_price_impact_too_high:{pi:.2f}%")
                                                                continue
                                                        except Exception:
                                                            pass
                                                    except Exception as e:
                                                        logger.debug("Inline Jupiter quote probe failed for %s: %s", token_addr, e, exc_info=True)
                                                        _log_skip(symbol, token_addr, "preflight_jupiter_probe_failed")
                                                        continue
                                            except Exception:
                                                logger.debug("Jupiter inline probe raised unexpectedly for %s; skipping candidate", token_addr, exc_info=True)
                                                _log_skip(symbol, token_addr, "preflight_probe_exception")
                                                continue

                                            # --- Dynamic ATA / token-program detection (best-effort, non-fatal) ---
                                            try:
                                                # advisory detection only. function signatures vary across helpers,
                                                # so call defensively and do NOT change behavior based on failure.
                                                if "token_program_id" not in tok:
                                                    try:
                                                        detected = await get_token_program_id(token_addr, solana_client)
                                                        if detected:
                                                            tok["token_program_id"] = str(detected)
                                                            if detected != globals().get("TOKEN_PROGRAM_ID"):
                                                                logger.info("Detected non-legacy token program for %s: %s", token_addr, detected)
                                                    except Exception:
                                                        # detection is advisory only
                                                        logger.debug("Token program detection failed for %s", token_addr, exc_info=True)

                                                # try to derive ATA if helper available (signature may vary)
                                                try:
                                                    ata_pub = None
                                                    # try several common helper signatures
                                                    if callable(globals().get("get_associated_token_address")):
                                                        g = globals().get("get_associated_token_address")
                                                        try:
                                                            # new-style: await get_associated_token_address(owner=<Pubkey>, mint=<str|Pubkey>, solana_client=...)
                                                            ata_pub = await g(owner=wallet.pubkey(), mint=token_addr, solana_client=solana_client)
                                                        except TypeError:
                                                            try:
                                                                # older signature: await get_associated_token_address(owner, mint)
                                                                ata_pub = await g(wallet.pubkey(), token_addr)
                                                            except Exception:
                                                                ata_pub = None
                                                    if ata_pub:
                                                        try:
                                                            resp = await solana_client.get_account_info(ata_pub)
                                                            if getattr(resp, "value", None) is None:
                                                                logger.info("[%s] ATA does not exist — Jupiter/AMM may create it at swap time", symbol)
                                                            else:
                                                                logger.debug("[%s] ATA exists", symbol)
                                                        except Exception:
                                                            logger.debug("Failed to check ATA account existence for %s", token_addr, exc_info=True)
                                                except Exception:
                                                    logger.debug("ATA derivation helper missing or failed for %s", token_addr, exc_info=True)
                                            except Exception:
                                                logger.debug("Dynamic ATA/token-program detection failed (continuing): %s", token_addr, exc_info=True)
                                            ...
                                            if txid:
                                                ...
                                                # Do NOT blacklist here — transient failures are common (RPC, blockhash, rate-limit)
